...
LDFLAGS="-w -X github.com/dennisrutjes/tegola/cmd/tegola/cmd.Version=${VERSION_TAG}"
...
build_bins(){
	...
	do
		...
		do
			...
			if [[ "$CGO_ENABLED" != "0" ]]; then
				echo "CGO_ENABLED: $CGO_ENABLED"
				xgo -go 1.10.x --targets="${GOOS}/${GOARCH}" -ldflags "${LDFLAGS}" -dest "${TRAVIS_BUILD_DIR}/releases" github.com/dennisrutjes/tegola/cmd/tegola
				mv ${TRAVIS_BUILD_DIR}/releases/tegola-${GOOS}* "${TRAVIS_BUILD_DIR}/releases/tegola${EXT}"
			else
				GOOS=${GOOS} GOARCH=${GOARCH} go build -ldflags "${LDFLAGS}" -o "${TRAVIS_BUILD_DIR}/releases/tegola${EXT}" github.com/dennisrutjes/tegola/cmd/tegola
...
build_lambda() {	
	...
	GOOS="linux" GOARCH="amd64" go build -ldflags "${LDFLAGS}" -o "${TRAVIS_BUILD_DIR}/releases/tegola_lambda" github.com/dennisrutjes/tegola/cmd/tegola_lambda
